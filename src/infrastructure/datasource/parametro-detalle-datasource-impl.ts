...
export class ParametroDetalleDataSourceImpl implements ParametroDetalleDataSource {
    ...
    async create( parametroDetalle: ParametroDetalle ): Promise<ParametroDetalle> {
        const query = `INSERT INTO parametros_detalle (
            id_parametro_general, nombre, codigo, orden, valores,
        ...
        const values = [
            ...
            parametroDetalle.valores, parametroDetalle.estado, parametroDetalle.creado_por
        ];
    ...
    async updateById( id_parametro_detalle: string, parametroDetalle: ParametroDetalle ): Promise<ParametroDetalle> {
        const result = await this.pool.query(
            'UPDATE parametros_detalle SET nombre = $2, codigo = $3, orden = $4, valores = $5, estado = $6, modificado_por = $7, fecha_modificacion = NOW() WHERE id_parametro_detalle = $1 RETURNING *', [
            ...
            parametroDetalle.modificado_por,
        ]);
        return result.rows[0];
    }  
