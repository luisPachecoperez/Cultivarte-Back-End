...
export class PoblacionDataSourceImpl implements PoblacionDataSource {
    ...
    async getPoblacionById(id_poblacion: string): Promise<Poblacion | RespuestaGrap> {
        try {
            const result = await this.pool.query(poblacionesQueries.poblacionById, [id_poblacion]);
            ...
            console.error('Error en getPoblacionById:', error);
            return {
                exitoso: "N",
                mensaje: 'Error al obtener la población: ' + error
            };
        }
    }
    ...
    async createPoblacion(poblacion: Poblacion): Promise<RespuestaGrap> {
        try {
            const result = await this.pool.query( 
                poblacionesQueries.createPoblacion, 
                [poblacion.id_poblacion || randomUUID(), poblacion.id_padre || null, poblacion.nombre] 
            );
            ...
            console.error('Error en createPoblacion:', error);
            return {
                exitoso: "N",
                mensaje: 'No se pudo crear la población: ' + error
            };
        }
    }       
