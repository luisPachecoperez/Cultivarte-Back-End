...
const controller = new SedesController( getSedesUseCase,
                                        getSedeUseCase,
                                        createSedeUseCase,
                                        updateSedeUseCase,
                                        deleteSedeUseCase );        
...
export const sedeResolvers = {
    Query: {
        ...
        getSedeById: (_: any, args: { id_sede:string }) => controller.getById( args.id_sede ),
    },
    Mutation: {
        createSede: (_: any, args: { data:Sede }) => controller.create( args.data ),
        updateSede: (_: any, args: { id_sede:string, data:Sede }) => controller.update( args.id_sede, args.data ),
        deleteSede: (_: any, args: { id_sede:string }) => controller.delete( args.id_sede )
    }
}