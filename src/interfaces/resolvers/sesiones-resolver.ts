...
export const controller = new SesionesController(
  useCaseGetAll,
  useCaseGetById,
...
export const sesionesResolvers = {
  Query: {
    getSesiones: (_: any, args: { limit: number; offset: number }) =>
      controller.getSesiones(args.limit, args.offset),
    getSesion: (_: any, args: { id_sesion: string }) =>
      controller.getSesion(args.id_sesion),
    getSesionesSedes: (
      _: any,
      args: { id_usuario: string; fecha_inicio: string; fecha_fin: string },
    ) =>
      controller.getSesionesSedes(
        args.id_usuario,
        args.fecha_inicio,
        args.fecha_fin,
      ),
  ...
  Mutation: {
    createSesion: (_: any, args: { input: Sesion }) =>
      controller.createSesion(args.input),
    updateSesion: (_: any, args: { input: Sesion }) =>
      controller.updateSesion(args.input.id_sesion, args.input),
    deleteSesion: (_: any, args: { id_sesion: string }) =>
      controller.deleteSesion(args.id_sesion),
    updateSesiones: (_: any, args: { input: EditarSesiones }) =>
      controller.updateSesiones(args.input),
  },
};
