...
export const asistenciasResolvers = {
  Query: {
    ...
    getAsistencia: (_: any, args: { id: string }) =>
      controller.getAsistencia(args.id),
    getPreAsistencia: (_: any, args: { id_sesion: string }) =>
      controller.getPreAsistencia(args.id_sesion),
    getAsistenciasSede: (
      _: any,
      args: { id_usuario: string; fecha_inicio: string; fecha_fin: string },
    ) =>
      controller.getAsistenciasSede(
        args.id_usuario,
        args.fecha_inicio,
        args.fecha_fin,
      ),
  ...
  Mutation: {
    createAsistencia: (_: any, args: { data: Asistencia }) =>
      controller.createAsistencia(args.data),
    updateAsistencia: (_: any, args: { id: string; data: Asistencia }) =>
      controller.updateAsistencia(args.id, args.data),
    deleteAsistencia: (_: any, args: { id: string }) =>
      controller.deleteAsistencia(args.id),
    updateAsistencias: (_: any, args: { input: AsistenciaSesiones }) =>
      controller.updateAsistencias(args.input),
  },
};
