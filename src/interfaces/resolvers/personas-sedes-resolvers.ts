...
const personasSedesRepository = new PersonasSedesRepositoryImpl(
  personasSedesDataSource,
);

const getPersonaSedeUseCase = new GetPersonaSedeUseCaseImpl(
  personasSedesRepository,
);
const getPersonasSedesUseCase = new GetPersonasSedesUseCaseImpl(
  personasSedesRepository,
);
const createPersonaSedeUseCase = new CreatePersonaSedeUseCaseImpl(
  personasSedesRepository,
);
const updatePersonaSedeUseCase = new UpdatePersonaSedeUseCaseImpl(
  personasSedesRepository,
);
const deletePersonaSedeUseCase = new DeletePersonaSedeUseCaseImpl(
  personasSedesRepository,
);

export const controller = new PersonasSedesController(
...
export const personasSedesResolvers = {
  ...
  Mutation: {
    createPersonaSede: (_: any, args: { personasSede: PersonasSede }) =>
      controller.create(args.personasSede),
    updatePersonaSede: (
      _: any,
      args: { id_sede: string; personasSede: PersonasSede },
    ) => controller.update(args.id_sede, args.personasSede),
    deletePersonaSede: (_: any, args: { id_sede: string }) =>
