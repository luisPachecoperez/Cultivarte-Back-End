...
describe('SesionesController', () => {
  ...
  const controller = new SesionesController(
    getSesionesUseCase as GetSesionesUseCase,
    getSesionUseCase as GetSesionUseCase,
    getSesionesSedesUseCase as GetSesionesSedesUseCase,
    createSesionUseCase as CreateSesionUseCase,
    updateSesionUseCase as UpdateSesionUseCase,
    deleteSesionUseCase as DeleteSesionUseCase,
    updateSesionesUseCase as UpdateSesionesUseCase,
  );

  const mockSesion: Sesion = {
    id_sesion: '1',
    id_actividad: '1',
  ...
  const mockEditarSesiones: EditarSesiones = {
    sesiones: {
      nuevos: [],
      modificados: [],
      eliminados: [],
  ...
  it('getSesionesSedes - success', async () => {
    ...
    await expect(
      controller.getSesionesSedes('user', '2024-01-01', '2024-01-31'),
    ).resolves.toEqual([mockSesion]);
    expect(getSesionesSedesUseCase.execute).toHaveBeenCalledWith(
      'user',
      '2024-01-01',
      '2024-01-31',
    );
  ...
  it('createSesion - success', async () => {
    ...
    await expect(controller.createSesion(mockSesion)).resolves.toBe(
      mockRespuesta,
    );
  ...
  it('updateSesion - success', async () => {
    ...
    await expect(controller.updateSesion('1', mockSesion)).resolves.toBe(
      mockRespuesta,
    );
  ...
  it('updateSesiones - success', async () => {
    ...
    await expect(controller.updateSesiones(mockEditarSesiones)).resolves.toBe(
      mockRespuesta,
    );
    expect(updateSesionesUseCase.execute).toHaveBeenCalledWith(
      mockEditarSesiones,
    );
  });
});
