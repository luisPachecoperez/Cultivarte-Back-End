...
describe('ActividadesController test', () => {
    ...
    it('getPreCreateActividad - success', async () => {
        ...
        await expect(controller.getPreCreateActividad('user')).resolves.toBe(mockPreCreate);
    ...
    it('getPreCreateActividad - error', async () => {
        ...
        await expect(controller.getPreCreateActividad('user')).resolves.toBe(mockRespuesta);
    });
    it('getPreEditActividad - success', async () => {
        getPreEditActividadUseCase.execute.mockResolvedValueOnce(mockPreEdit);
        await expect(controller.getPreEditActividad('act', 'user')).resolves.toBe(mockPreEdit);
        expect(getPreEditActividadUseCase.execute).toHaveBeenCalledWith('act', 'user');
    });
    it('getPreEditActividad - error', async () => {
        getPreEditActividadUseCase.execute.mockResolvedValueOnce(mockRespuesta);
        await expect(controller.getPreEditActividad('act', 'user')).resolves.toBe(mockRespuesta);
    });
    ...
    it('getActividadSedes - success', async () => {
        ...
        await expect(controller.getActividadSedes('user', '2024-01-01', '2024-01-31')).resolves.toEqual([mockActividad]);
        expect(getActividadSedesUseCase.execute).toHaveBeenCalledWith('user', '2024-01-01', '2024-01-31');
    });
    it('getActividadSedes - error', async () => {
        getActividadSedesUseCase.execute.mockResolvedValueOnce(mockRespuesta);
        await expect(controller.getActividadSedes('user', '2024-01-01', '2024-01-31')).resolves.toBe(mockRespuesta);
    });
    it('createActividadAndSesiones - success', async () => {
        createActividadAndSesionesUseCase.execute.mockResolvedValueOnce(mockActividad);
        await expect(controller.createActividadAndSesiones(mockActividad)).resolves.toBe(mockActividad);
        expect(createActividadAndSesionesUseCase.execute).toHaveBeenCalledWith(mockActividad);
    });
    it('createActividadAndSesiones - error', async () => {
        createActividadAndSesionesUseCase.execute.mockResolvedValueOnce(mockRespuesta);
        await expect(controller.createActividadAndSesiones(mockActividad)).resolves.toBe(mockRespuesta);
    });
    it('createActividad - success', async () => {
        createActividadUseCase.execute.mockResolvedValueOnce(mockActividad);
        await expect(controller.createActividad(mockActividad)).resolves.toBe(mockActividad);
    ...
    it('createActividad - error', async () => {
        ...
        await expect(controller.createActividad(mockActividad)).resolves.toBe(mockRespuesta);
    });
    it('updateActividad - success', async () => {
        updateActividadUseCase.execute.mockResolvedValueOnce(mockActividad);
        await expect(controller.updateActividad('1', mockActividad)).resolves.toBe(mockActividad);
        expect(updateActividadUseCase.execute).toHaveBeenCalledWith('1', mockActividad);
    });
    it('updateActividad - error', async () => {
        updateActividadUseCase.execute.mockResolvedValueOnce(mockRespuesta);
        await expect(controller.updateActividad('1', mockActividad)).resolves.toBe(mockRespuesta);
    });
...
//# sourceMappingURL=actividad-controller.spec.js.map