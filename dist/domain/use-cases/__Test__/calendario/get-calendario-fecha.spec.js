...
const get_calendario_fecha_1 = require("../../calendario/get-calendario-fecha");
describe('GetCalendarioFechaUseCaseImpl', () => {
    const calendarioFechaRepository = {
        getByDate: jest.fn(),
    };
    const useCase = new get_calendario_fecha_1.GetCalendarioFechaUseCaseImpl(calendarioFechaRepository);
    const mockInput = {
    ...
    it('propaga errores si ocurren', async () => {
        calendarioFechaRepository.getByDate.mockRejectedValueOnce(new Error('DB error'));
        await expect(useCase.execute(mockInput)).rejects.toThrow('DB error');
    });
});
//# sourceMappingURL=get-calendario-fecha.spec.js.map