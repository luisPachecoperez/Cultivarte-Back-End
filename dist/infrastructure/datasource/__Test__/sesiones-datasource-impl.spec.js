...
it('createSesion retorna error si ocurre excepción no Error', async () => {
    ...
    const result = await dataSource.createSesion(sesion);
    expect(result.exitoso).toBe('N');
    expect(result.mensaje).toMatch(/custom/);
});
it('updateById retorna error si ocurre excepción no Error', async () => {
    ...
    const result = await dataSource.updateById('1', sesion);
    expect(result.exitoso).toBe('N');
    expect(result.mensaje).toMatch(/custom/);
});
...
const sesiones_datasource_impl_1 = require("../sesiones-datasource-impl");
...
describe('SesionesDataSourceImpl', () => {
    let dataSource;
    beforeEach(() => {
        dataSource = new sesiones_datasource_impl_1.SesionesDataSourceImpl();
        jest.clearAllMocks();
    });
    it('getAll retorna lista correctamente', async () => {
        pg_pool_1.pgPool.query.mockResolvedValue({ rows: [{ id_sesion: '1' }] });
        const result = await dataSource.getAll(10, 0);
    ...
    it('updateSesiones actualiza correctamente (todas las ramas)', async () => {
        ...
        const result = await dataSource.updateSesiones(editarSesiones);
        expect(result.exitoso).toBe('S');
    ...
    it('updateSesiones maneja error en release', async () => {
        ...
        mockClient.query.mockResolvedValue({});
        const editarSesiones = { sesiones: { nuevos: [], modificados: [], eliminados: [] } };
        await expect(dataSource.updateSesiones(editarSesiones)).resolves.toBeTruthy();
    });
    ...
    it('updateSesiones retorna error si editarSesiones no tiene sesiones', async () => {
        ...
        pg_pool_1.pgPool.connect.mockResolvedValue(mockClient);
        mockClient.query.mockResolvedValue({});
        const result = await dataSource.updateSesiones({ sesiones: {} });
        expect(result.exitoso).toBe('N');
    ...
    it('updateSesiones retorna error si editarSesiones no tiene sesiones', async () => {
        ...
        pg_pool_1.pgPool.connect.mockResolvedValue(mockClient);
        mockClient.query.mockResolvedValue({});
        const result = await dataSource.updateSesiones({});
        if ('exitoso' in result) {
            expect(result.exitoso).toBe('N');
    ...
    it('updateSesiones retorna error si el error no es instancia de Error', async () => {
        ...
        mockClient.query.mockImplementationOnce(() => { throw { custom: 'fail' }; });
        mockClient.query.mockResolvedValueOnce({});
        const editarSesiones = {
            sesiones: {
                nuevos: [
...
//# sourceMappingURL=sesiones-datasource-impl.spec.js.map