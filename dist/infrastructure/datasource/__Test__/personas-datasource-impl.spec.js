...
const personas_datasource_impl_1 = require("../personas-datasource-impl");
...
describe('PersonaDataSourceImpl', () => {
    let dataSource;
    beforeEach(() => {
        dataSource = new personas_datasource_impl_1.PersonaDataSourceImpl();
        jest.clearAllMocks();
    });
    it('getAll retorna lista correctamente', async () => {
        pg_pool_1.pgPool.query.mockResolvedValue({ rows: [{ id_persona: '1' }] });
    ...
    it('createPersona retorna persona correctamente', async () => {
        ...
        const result = await dataSource.createPersona(persona);
        expect(result).toHaveProperty('id_persona', '1');
    });
    ...
    it('updatePersona retorna persona correctamente', async () => {
        pg_pool_1.pgPool.query.mockResolvedValue({ rows: [{ id_persona: '1' }] });
        const persona = { id_persona: '1' };
        const result = await dataSource.updatePersona('1', persona);
        expect(result).toHaveProperty('id_persona', '1');
    });
...
//# sourceMappingURL=personas-datasource-impl.spec.js.map